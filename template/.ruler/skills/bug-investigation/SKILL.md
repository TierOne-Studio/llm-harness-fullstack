...
├── UI / component / hook                         → check rendered DOM, props, state, effect deps (frontend, commonly apps/web)
├── Shared contract / type                        → check the contract package both tiers import (commonly packages/contracts) for a breaking change reflected on only one side
