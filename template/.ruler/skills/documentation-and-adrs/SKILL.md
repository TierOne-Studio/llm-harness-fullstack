...
- A user asks to make a structural choice that will constrain future code. On the frontend (commonly `apps/web`): state-management library, server-state library, auth library, styling system. On the backend (commonly `apps/api`): persistence pattern, error-mapping convention, auth library. Across the seam: the public-API contract shape / shared types (commonly `packages/contracts`).
...
3. Citation site (depending on type):
   ...
   |---|---|
   | Frontend convention (state-mgmt, server-state, styling, forms, testing, auth-token storage) | `repo-conventions` § "ADR-backed conventions" table |
   | Backend convention (persistence, error-handling, logging, DTOs, layering) | `repo-conventions` § "ADR-backed conventions" table |
   | Shared-contract convention (public-API shape, shared types) | `repo-conventions` § "ADR-backed conventions" table |
...
> ✅ (frontend) "Per `ADR-00X`, the auth token lives in `localStorage` because the auth flow requires header-based delivery and the session-cookie alternative was rejected for cross-origin reasons."
> ✅ (backend) "Per `ADR-00X`, throw NestJS built-in exceptions; no global filter."
