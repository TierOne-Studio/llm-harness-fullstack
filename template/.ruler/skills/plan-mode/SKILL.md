...
Before writing the per-step plan, sketch what depends on what. The dependency graph dictates implementation order — foundations first, consumers last. In a fullstack monorepo (commonly `apps/web` for the React frontend, `apps/api` for the NestJS backend, `packages/contracts` for shared types), the shared contract sits at the root of the graph and both tiers depend on it; change the contract first, then propagate to both sides.

For the backend tier (commonly `apps/api`), the typical layering is:
...
For the frontend tier (commonly `apps/web`), the typical layering is:

```
Schema / type / API contract (shared in packages/contracts, server-side, or zod schema)
    │
...
When a change spans the FE↔BE seam, the shared type in `packages/contracts` is the common foundation: plan the contract change first, then the backend that produces it, then the frontend that consumes it, and end-to-end (`e2e/`, commonly Playwright) tests that exercise the seam.
...
- **Vertical (default — tracer bullet).** Each slice cuts through every layer end-to-end for ONE narrow path — backend (entity + repo + service + controller + test), frontend (schema + service + query hook + component + test), or a full FE↔BE path through the shared contract. Pairs with the ~100-LOC cap. Best when the layer-stack is well-understood and the risk is in the integration.
...
- **Contract-first.** When a public API or module boundary is being introduced, **Slice 0 = define the contract** (types / interface / OpenAPI surface — commonly the shared types in `packages/contracts`). Then Slice 1+ implements behind the contract; consumers can develop in parallel against the same shape. Best fit for new NestJS controllers exposing a route the frontend or another service will consume, and for new feature contracts the BFF/API will serve to the frontend.

State the choice in the plan output (e.g., backend: `Slicing: contract-first — Slice 0 defines the DTOs and controller signatures; Slice 1 implements the service`; frontend: `Slicing: contract-first — Slice 0 defines the shared types and route signatures; Slice 1 implements the hook`). The reviewer (`architect-reviewer`) checks whether the choice matches the actual risk profile.

### Step sizing — tracer-bullet vertical slices (~100 LOC cap)

Each step is a **tracer-bullet vertical slice**: a thin path that cuts through every layer end-to-end (backend: schema → service → controller → test; frontend: schema → service → query hook → component → test), NOT a horizontal slice of one layer. A completed slice is demoable or verifiable on its own. Implementable, testable, and committable on its own. Target ≤ ~100 LOC of executable code per step (tests excluded from the count). If a step's implementation crosses ~100 LOC mid-execution, **STOP, commit what's working, and split the rest into a new step.** Don't push through.
