...
Example (frontend change, commonly under `apps/web`):
```
...
Example (backend change, commonly under `apps/api`):
```
Awaiting approval for: git push origin feat/scope-fix
Branch:        feat/scope-fix (local) → origin/feat/scope-fix (remote)
...
A change to a shared contract (commonly `packages/contracts`) typically touches both tiers — list the files from `apps/web`, `apps/api`, and `packages/contracts` together and call out that the change crosses the FE↔BE seam.
