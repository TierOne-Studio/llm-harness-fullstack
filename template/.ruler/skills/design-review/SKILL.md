...
```ts
// Frontend (apps/web) — Bad: what does this *actually* do?
function projectHelper(p: Project) { ... }
function stringUtil(s: string) { ... }
function useDataManager() { ... }

// Frontend (apps/web) — Good: name describes the responsibility
function useArchivedProjectsForCurrentOrg() { ... }
function slugify(name: string) { ... }
function useChatConversationList() { ... }
```
```ts
// Backend (apps/api) — Bad: what does this *actually* do?
class ProjectManager { ... }
class AuthHelper { ... }
function stringUtil(s: string) { ... }

// Backend (apps/api) — Good: name describes the responsibility
...
```ts
// Frontend (apps/web) — Bad: generic hook for ONE concrete caller
...
// Frontend (apps/web) — Good: concrete now, generalize when caller #2 appears
function useProjects() {
  return useQuery({ queryKey: projectKeys.all, queryFn: fetchProjects })
}
```
```ts
// Backend (apps/api) — Bad: generic interface for ONE concrete caller
...
// Backend (apps/api) — Good: concrete now, generalize when caller #2 appears
