...
description: 'Simplify and refine recently modified code for clarity, consistency, and maintainability while preserving exact behavior. Use after edits, during cleanup, or when reviewing touched files across the fullstack monorepo — frontend (commonly `apps/web`), backend (commonly `apps/api`), and shared contracts (commonly `packages/contracts`). Prefer explicit readable code, follow project standards, avoid nested ternaries, and keep scope limited to changed code unless asked otherwise.'
...
- For top-level APIs and components — frontend React components and hooks as well as backend NestJS controllers, services, and providers — keep types explicit when the project conventions require them.
