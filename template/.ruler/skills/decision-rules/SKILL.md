---
name: decision-rules
description: Use when interpreting an ambiguous request, when scope is unclear, when choosing between two libraries/approaches/options, when the user says "talk me out of it" or asks whether a proposed choice is wrong, when a test fails but looks wrong, when CLAUDE.md and a skill disagree, or whenever a default decision is needed. NOT for unambiguous requests, code-quality questions, or workflow procedures.
harness:
  tier: shared
  family: process
...
**Situation:** User asks you to do X but X violates a `repo-conventions` rule — e.g. (frontend) "store this token in a new place" when the project already has a documented token-storage approach, or "throw a generic `Error`" when the convention is to surface via a typed error / toast / error boundary; (backend) "throw a generic `Error`" when the convention is NestJS exceptions, or "use raw SQL" in a new module without one of the documented justifications for falling back from TypeORM.
