...
How `@tierone/llm-harness-fullstack` works under the hood: what the moving parts
...
```mermaid
flowchart TB
    subgraph PKG["npm package @tierone/llm-harness-fullstack"]
        direction TB
    ...
    subgraph CONSUMER["Consumer monorepo (apps/api + apps/web)"]
...
sequenceDiagram
    ...
    U->>C: npx llm-harness-fullstack init
    C->>I: init({cwd, force})
...
sequenceDiagram
    ...
    U->>C: npx llm-harness-fullstack update [--dry-run]
...
flowchart LR
    ...
    subgraph PROFILE["instructions.md — operating profile (always in context)"]
        ...
        P3["P3 Code-change defaults — tier routing,<br/>TDD, P3.4 force-fire skill matrix,<br/>P3.6 fast/full path declaration"]
        P4["P4 Mandatory verification —<br/>spawn review subagents"]
        P8["P8 Output contract —<br/>executed-verification line"]
        P0 --> P3 --> P4 --> P8
    end
...
deliberate *instruction diet* — a compact always-on profile with priority
ordering (`P0 > P1 > … > P9`, lower number wins on conflict), skills loaded
lazily by description match, a **force-fire matrix (P3.4)** for the skills whose
description-trigger is too unreliable to trust, and deterministic gates (CI,
pre-commit, permission rules) as the backstop for whatever advice the model
drops on the floor.
...
1. **Force-fire (P3.4)** — a 12-row matrix of skills the model MUST load
   ...
   3+ steps/multi-file/architectural work, plus tier-conditional rows
   (`react-patterns`, `react-state-management`, `accessibility`,
   `async-error-handling`, `database-transactions`, `spec-workflow`,
   `cross-repo-workspace`). If a force-fire skill genuinely doesn't apply, the
   agent must say so in a literal waiver line (`react-patterns waived —
   backend-only change`) — **silent omission is a P8 contract violation**.
...
All three are constrained by **tier routing (P3.0)**: identify which
workspace(s) the diff touches first, then load only that tier's skills — a
backend-only change must never pull `react-*` into context. This is the
instruction-diet principle applied to skills themselves.

```mermaid
flowchart TB
    REQ["Code task"] --> PATH["P3.6 — declare<br/><b>Path: fast | full</b><br/>(literal FIRST line, auditable)"]
    PATH --> TIER["P3.0 tier routing — which<br/>workspaces does the change touch?"]
    TIER -->|apps/web| FE["frontend skill set"]
    TIER -->|apps/api| BE["backend skill set"]
    TIER -->|packages/contracts| SEAM["contract seam +<br/>consuming tier(s)"]
    TIER -->|always| SH["shared/process skills<br/>(stack-neutral)"]
    FE & BE & SEAM & SH --> LOAD

    subgraph LOAD["Loading mechanisms (within the routed tier)"]
        direction TB
...
| [design-sync](../template/.ruler/agents/design-sync.md) | PRE **and** POST | cross-tier behavior or layer docs changed | backend/frontend/shared-contract agreement on behavior, data, errors, auth, UI states, proving layer | sync matrix | Read, Grep, Glob — read-only |
...
| [acceptance-verifier](../template/.ruler/agents/acceptance-verifier.md) | POST-impl, **always LAST** (after qa-validator green) | user-facing/API feature OR behavior-altering bug fix | DYNAMIC proof: runs the live suite, maps every acceptance criterion to an EXECUTED assertion | ACCEPTED / GAPS / BLOCK — **binding on "done" (P8.0)** | read-only + Bash (runs Playwright / Jest vs real Postgres) |
...
- **Tier lenses follow the diff.** Every reviewer applies the same P3.0 routing
  the main agent used: frontend lens for `apps/web`, backend lens for
  `apps/api`, shared-contract lens for `packages/contracts` (where a breaking
  type change unreflected on both producer and consumer sides is itself a
  finding) — never both stacks for a single-tier diff.
- **Static vs dynamic split.** The three mid-fleet reviewers reason
  *statically over the diff*; `acceptance-verifier` exists because static
  ...
  and a migration test asserting on SQL *string shape* without executing it).
  It re-runs the suites and adversarially checks **non-vacuity**: would this
  green test go red if the feature were reverted? A test that can't fail is
  `DRIFTED`, not `PASS`.
...
flowchart LR
    ...
    RC -- "layout/authz/persistence<br/>conventions confirmed per repo" --> AR & CRV & QA & SECR["security-reviewer"]
...
- `architect-reviewer` critiques the plan "against the MUST principles in the
...
- `qa-validator` checks edge cases "per `failure-mode-analysis` (8 categories):
  null, empty, very large, boundary, off-by-one, async race, partial, timezone,
  locale, encoding".
...
- `spec-steward`: "procedure lives in the `spec-workflow` skill; this file is
  ...
  defines the how.
- All reviewers treat `apps/web`/`apps/api`/`packages/contracts` as "common
  convention, never a mandate; confirm the actual layout from
  `repo-conventions`" — the fill-in skill is the single source for what this
  particular repo actually does.

### 3.4 Full-path lifecycle — one feature, end to end

The *Workflow chains* section of the profile turns all of the above into an
ordered recipe per task type. Here is the **full-stack feature** chain
(endpoint + UI + shared contract), the richest one:
...
sequenceDiagram
    ...
    U->>M: "Add org-invite endpoint + invite UI"
    M->>M: «Path: full — touches packages/contracts» (P3.6, first line)
    M->>SK: P3.4 force-fire: spec-workflow, plan-mode,<br/>failure-mode-analysis, tdd-workflow, repo-conventions…

    rect rgb(235, 242, 255)
        Note over M,AR: PRE-implementation gates
        M->>SS: spawn PRE — clarification gate + SPEC
        ...
        SS-->>M: SPEC ready (ui + contract specs, cross-linked)
        M->>AR: spawn with the plan (3+ files / contract change)
        Note over AR: critiques plan vs design-review MUSTs<br/>+ repo-conventions, all three tier lenses
        alt REVISE_PLAN
            AR-->>M: findings
            M->>AR: revised plan
        end
    ...
    rect rgb(235, 250, 238)
        Note over M,SK: implementation — contract first, then per-tier TDD slices
        M->>M: 1 · shared type in packages/contracts (the seam)
        M->>SK: 2 · backend slice: tdd-workflow + nestjs-* +<br/>database-transactions (red test → impl → green)
        M->>SK: 3 · frontend slice: tdd-workflow + react-* +<br/>accessibility self-check
        M->>M: 4 · Playwright seam test in e2e/ → both suites green
    ...
    rect rgb(255, 244, 235)
        ...
        and
            M->>SEC: diff → security verdict (RBAC touched)
        end
        CR-->>M: APPROVE
        QA-->>M: green
        SEC-->>M: green
    ...
    M-->>U: P8 output contract — path line, waivers,<br/>review verdicts, executed-verification line
...
`cyclomatic-complexity` and only needs `qa-validator` for coverage parity; a
**structural decision** pulls `documentation-and-adrs` and `architect-reviewer`
in together; a **docs/ADR-only change** waives TDD with the exact phrase and
spawns no subagents at all.
...
`Path: fast` when **all five** hold: ≤2 files · single tier · no high-risk
surface (P3.3) · no contract/schema change · no new dependency. The fast chain
is just `tdd-workflow` + `repo-conventions` + `design-review`, with
`qa-validator` only if observable behavior changes and no other subagents.
Crucially the fast path skips *subagents and the spec PRE gate* — **not** the
P3.4 force-load matrix and **not** waiver discipline. And it's self-escalating:
the moment a qualifying change stops qualifying (file count grows, a risk
surface gets touched), the agent must STOP, emit
`Path: full — escalated: <reason>`, and switch chains. The declaration is
auditable, so a wrong path claim is itself a P8 contract violation — which is
exactly the kind of literal, regex-checkable artifact `adherence-eval.mjs`
asserts on.
...
1. **Deterministic suites** (free, every PR, the CI gate):
   ...
   - `npm run test:harness` — [run-acceptance.sh](../template/.ruler/tests/run-acceptance.sh)
     validates the shipped tree structurally (both tiers covered, no
     project-specific coupling leaks) and
...
| Eval | Question | Gate |
|---|---|---|
| [routing-eval.mjs](../eval/routing-eval.mjs) | does a model, given the shipped catalog, route prompts (incl. paraphrases) to the right skills — and load NOTHING for pure questions? | worst-variant recall ≥ baseline − 0.05, FP rate ≤ baseline + 0.10 |
| [adherence-eval.mjs](../eval/adherence-eval.mjs) | under the full `instructions.md`, does the model emit the literal gates (approval pauses, waiver phrases, path declarations) — calm, multi-turn, and under injection pressure? | pass ≥ baseline − 0.10; **safety category: zero tolerance** |
| [mutation-test.mjs](../scripts/mutation-test.mjs) | *eval of the eval*: seed gate-deletions/MUST-softenings into a temp copy — do the suites go red? | kill rate = 1.0 |
...
pass on **majority vote** across `--repeats N` (single-shot is ±0.15 noisy on
the CLI backend); split votes are flagged as *marginal instructions*. A
per-category scorecard (safety / routing / ceremony / contract / identity)
gates safety regressions with zero tolerance.
...
flowchart LR
    ...
    CI -->|merge to main| REL["release.yml<br/>npm test → npm view name@version"]
    REL -->|version already on registry| NOOP["no-op"]
    REL -->|new version| PUB["npm publish --provenance"]
